...
<article id="post-<?php  echo $article->ID;  ?>" class="archive-list">
		...
		<div class="entry-content">
			...
			<span class="entry-meta">
				<span class="post_cat">
				<a href="<?php  echo $article->Category->Url;  ?>" target="_blank" rel="category tag"><?php  echo $article->Category->Name;  ?></a>			</span>
				<span class="post_spliter">•</span>
				<span class="date" title="<?php  echo $article->Time();  ?>"><?php echo zbpNana_timeago($article->Time()); ?></span>			
			</span>		
			
			<div class="archive-content">
			<?php if (strlen($article->Intro)>0) { ?>
				<?php $intro= trim(SubStrUTF8(TransferHTML($article->Intro,'[nohtml]'),80)).'...'; ?>
				<?php }else{  ?>
