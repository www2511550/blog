...
class timthumb {
        ...
        public function __construct(){
                        ...
                        if(ALLOW_ALL_EXTERNAL_SITES){
                                ...
                                foreach($ALLOWED_SITES as $site){
                                        if ((strtolower(substr($this->url['host'],-strlen($site)-1)) === strtolower(".$site")) || (strtolower($this->url['host'])===strtolower($site))) {
                                                $this->debug(3, "URL hostname <?php  echo $this->url['host'];  ?> matches $site so allowing.");
                                                $allowed = true;
                                        }
                                }
                                if(! $allowed){
                ...
                if($this->isURL){
                        ...
                        if(! $this->localImage){
                                $this->debug(1, "Could not find the local image: <?php  echo $this->localImage;  ?>");
                                $this->error("Could not find the internal image you specified.");
                                $this->set404();
                                return false;
                        }
                        $this->debug(1, "Local image path is <?php  echo $this->localImage;  ?>");
        ...
        protected function tryServerCache(){
                ...
                if(file_exists($this->cachefile)){
                        $this->debug(3, "Cachefile <?php  echo $this->cachefile;  ?> exists");
                        if($this->isURL){
                                ...
                                $this->debug(3, "Trying to serve cachefile <?php  echo $this->cachefile;  ?>");
                        }
                        if($this->serveCacheFile()){
                                $this->debug(3, "Succesfully served cachefile <?php  echo $this->cachefile;  ?>");
                                return true;
                        } else {
                                $this->debug(3, "Failed to serve cachefile <?php  echo $this->cachefile;  ?> - Deleting it from cache.");
        ...
        protected function serveWebshot(){
                ...
                if(WEBSHOT_XVFB_RUNNING){
                        ...
                        $command = "$xv --server-args=\"-screen 0, <?php  echo $screenX;  ?>x<?php  echo $screenY;  ?>x<?php  echo $colDepth;  ?>\" $cuty $proxy --max-wait=$timeout --user-agent=\"$ua\" --javascript=$jsOn --java=$javaOn --plugins=$pluginsOn --js-can-open-windows=off --url=\"$url\" --out-format=$format --out=$tempfile";
                }
        ...
        protected function serveCacheFile(){
                $this->debug(3, "Serving <?php  echo $this->cachefile;  ?>");
                if(! is_file($this->cachefile)){
